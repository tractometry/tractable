...
```{r fit no ac model}
gam_fit_cst_no_ac <- tractable_single_tract(
  df         = df_sarica,
...
```{r calculate rho}
rho <- start_value_rho(gam_fit_cst_no_ac)
start_value_rho(gam_fit_cst_no_ac, plot = TRUE)
```
...
gam_fit_cst <- tractable_single_tract(
  df         = df_sarica,
  tract      = "Right Corticospinal",
  target     = "fa",
  regressors = c("age", "group"),
